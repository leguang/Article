...
### 锁仓利息首页（Web实现）
摩尔钱包App点击“发现”模块中配置好的锁仓利息Item，将token和App版本信息通过url传参形式传给Web，再由熊老师放到请求头里（由于本来考虑到对公共参数统一放请求头中处理，因此这里稍微麻烦一点）请求地址：https://api.xxx.com/mol_wallet/v1/stake接口获取数据显示出来。底部的说明文字写死就好，就暂时不出接口配置了。

### 首页点击账单进入账单页面（Web实现）
请求地址：https://api.xxx.com/mol_wallet/v1/stake/records接口获取数据显示出来，注意请求头里的公共参数。

### 首页点击？icon进入规则明细（Web实现）
请求地址：https://api.xxx.com/mol_wallet/v1/stake/detail接口获取数据显示出来，注意请求头里的公共参数。
...
### 充币页面（原生实现）
1. 请求地址：https://api.xxx.com/mol_wallet/v1/stake/addresses
接口获取指定的打到官方的收款地址。在经过一系列的转账流程后，会从公链的接口得到这笔转账的block的hash值，作为参数，由于考虑到可能的网络问题，导致该地址获取不到，因此要做好二级缓存保证时刻有这个地址。

2. 接着请求地址：https://api.xxx.com/mol_wallet/v1/stake/deposit
...
### 提币页面（Web实现）
点击提币按钮后，跳转到Web的提币页面，请求地址：https://api.xxx.com/mol_wallet/v1/stake/withdraw 接口，以POST形式提交相关参数后跳转相应的页面展示结果即可。注意请求头里的公共参数。
