...
2. **接着走原生转账流程，但是由于考虑到双花问题，需要链上有回调告诉后台，因此我们另起一个特殊的action为process_stake，通过这个action来替换之前的process，这样可以将所有利息挖矿中充币转账的消息过滤出来通知到后台，因此前端要注意，在转账处理block时，将改用process_stake这个action，而不是之前的process。**

3. 接着请求 https://api.xxx.com/mol_wallet/v1/stake/deposit
接口，使用POST形式将参数都传递给后台，后台通过blocks_info这个action获取hash信息后对比，确认无误则返回200才表明此次充币完整无误。注意请求头里的公共参数。

4. 充币成功后，调用JS代码告诉Web已经充值成功，并隐藏该页面，否则就停留在充值页面，直到用户点击返回。JS方法名为deposited，以Android为例：
```
